...
use fhevm_engine_common::db_keys::{self, fetch_latest_db_key};
...
pub async fn execute_verify_proofs_loop(
    ...
    let host_chain_cache = HostChainsCache::load(&pool_mngr.pool())
        .await
...
async fn execute_worker(
    ...
    let latest_key = fetch_latest_db_key(&pool, &db_key_cache)
        .await
        .map_err(|err| {
    ...
    loop {
        ...
        execute_verify_proof_routine(&pool, &latest_key, &latest_crs, &host_chain_cache, &conf)
            .await?;
...
async fn execute_verify_proof_routine(
    ...
    .await
    {
        ...
        let host_chain = host_chain_cache
            .get_chain(chain_id)
            .ok_or(ExecutionError::InvalidChainId(chain_id))?;
        ...
        let res = tokio::task::spawn_blocking(move || {
            let aux_data = auxiliary::ZkData {
                ...
                user_address,
                chain_id,
                acl_contract_address: host_chain.acl_contract_address.clone(),
            };
