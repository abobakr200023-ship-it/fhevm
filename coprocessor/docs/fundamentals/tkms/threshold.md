...
- Proactive: it is possible to "undo" a leakage of key material of at most `t` parties byte refreshing their key shares. That is, if a few servers are compromised it is possible to make the stolen material 100% useless without the need to regenerate a new public key.
